...
スキル自身は Web アクセスを一切行いません。記録に登場する URL の取得や
ホストへの接続はゼロで、IoC エンリッチメントはこのスキルの外で意図的に
行う別工程です。なお分析は Claude セッション上で実行されるため、前処理済み
（defang + ノンス隔離済み）の記録は他のセッション内容と同様にセッションを
支えるモデルバックエンドへ送られます。それ以外の宛先には何も送られません。
...
- すべての出力内の IoC は defang 済み（`hxxps`, `[.]`）で、レポートは
  安全に共有・索引できます。
- 記録は機密として扱います。出力はローカルのレポートディレクトリに
  とどまり、記録の内容が届く先は Claude セッションを支えるモデル
  バックエンドのみです。
